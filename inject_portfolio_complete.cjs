...
<div id="portfolio-root">
  ...
  <section id="pf-hero" class="pf-section" style="text-align:center; padding-top:140px; padding-bottom:60px;">
    ...
    <div style="display:flex; gap:10px; justify-content:center; margin-top:36px; flex-wrap:wrap;">
      ...
      <a id="pf-btn-instagram" href="#" target="_blank" class="pf-btn pf-btn-outline">Instagram</a>
  ...
  <section id="pf-about" class="pf-section">
    ...
    <div class="pf-about-grid" style="display:grid; grid-template-columns:1.2fr 0.8fr; gap:48px;">
      ...
      <div id="pf-skills-container" style="display:flex; flex-direction:column; gap:24px; padding-top:4px;">
        <div>
  ...
  <section id="pf-contact-section" class="pf-section" style="text-align:center;">
    ...
    <div style="display:flex; gap:10px; justify-content:center; flex-wrap:wrap;">
      ...
      <a id="pf-c-instagram" href="#" target="_blank" class="pf-btn pf-btn-outline">Instagram</a>
    </div>
  </section>
...
(function(){
  ...
  var DEF = {
    ...
    skills: [
      { label: "Frontend / WebGL", value: 92, color: "green" },
      { label: "Backend / Web3", value: 84, color: "cyan" },
      { label: "Hardware / IoT", value: 89, color: "orange" }
    ],
    email: "iamudt19@gmail.com",
    github: "https://github.com/Iamudt19",
    linkedin: "https://linkedin.com",
    instagram: "https://www.instagram.com/_iamudit_02/",
    projects: [
  ...
  function render() {
    ...
    $('pf-btn-instagram').href = d.instagram || 'https://www.instagram.com/_iamudit_02/';
    ...
    if (Array.isArray(d.skills) && d.skills.length) {
      ...
      skillsArr = [
        { label: 'Frontend / WebGL', value: d.skill1 || DEF.skill1, color: 'green' },
        { label: 'Backend / Web3', value: d.skill2 || DEF.skill2, color: 'cyan' },
        { label: 'Hardware / IoT', value: d.skill3 || DEF.skill3, color: 'orange' }
      ];
    }
    var skillsContainer = $('pf-skills-container');
    if (skillsContainer) {
      skillsContainer.innerHTML = '';
      skillsArr.forEach(function(skill, index) {
        var skillDiv = document.createElement('div');
        var fillStyle = 'background-color: var(--text-primary) !important;';
        if (skill.color) {
          if (skill.color === 'green') fillStyle = 'background-color: var(--term-green) !important;';
          else if (skill.color === 'cyan') fillStyle = 'background-color: var(--term-cyan) !important;';
          else if (skill.color === 'yellow') fillStyle = 'background-color: var(--term-yellow) !important;';
          else if (skill.color === 'orange') fillStyle = 'background-color: var(--term-orange) !important;';
          else if (skill.color === 'red') fillStyle = 'background-color: var(--term-red) !important;';
          else if (skill.color === 'magenta' || skill.color === 'purple') fillStyle = 'background-color: var(--term-magenta) !important;';
          else if (skill.color === 'blue') fillStyle = 'background-color: var(--term-blue) !important;';
          else if (skill.color === 'white') fillStyle = 'background-color: var(--term-white) !important;';
        }

        skillDiv.innerHTML = [
          '  <div style="display:flex; justify-content:space-between; margin-bottom:10px;">',
          '    <span style="font-size:10px; font-weight:700; text-transform:uppercase; letter-spacing:0.12em; color:var(--text-secondary);">' + skill.label + '</span>',
          '    <span id="pf-s' + index + 'v" class="font-mono" style="font-size:10px; font-weight:700;">0%</span>',
          '  </div>',
          '  <div class="pf-skill-track"><div id="pf-s' + index + 'b" class="pf-skill-fill" style="width:0%; ' + fillStyle + '"></div></div>'
        ].join(' ');
        skillsContainer.appendChild(skillDiv);

        setTimeout(function() {
          var valEl = $('pf-s' + index + 'v');
          var barEl = $('pf-s' + index + 'b');
          if (valEl) valEl.textContent = skill.value + '%';
    ...
    $('pf-c-instagram').href = d.instagram || 'https://www.instagram.com/_iamudit_02/';
    ...
    var cmds = {
      ...
      skills: function(){
        ...
        function bar(label,val,color){
          ...
          if (paddedLabel.length < 20) {
            paddedLabel = paddedLabel + Array(20 - paddedLabel.length + 1).join(' ');
          } else {
            paddedLabel = paddedLabel.substring(0, 17) + '...';
          }
          var termColor = 't-white';
          if (color === 'green') termColor = 't-green';
          else if (color === 'cyan') termColor = 't-cyan';
          else if (color === 'orange' || color === 'yellow') termColor = 't-yellow';
          else if (color === 'red') termColor = 't-red';
          else if (color === 'magenta' || color === 'purple') termColor = 't-magenta';
          else if (color === 'blue') termColor = 't-blue';
          pr(c('  │ ', 't-dim') + c(paddedLabel, 't-white') + c(' ' + barStr + ' ', termColor) + c(val+'%', 't-white'));
        }
        skillsArr.forEach(function(skill) {
          bar(skill.label, skill.value, skill.color);
        });
        pr(c('  └─────────────────────────────────────', 't-dim'));
      },
      projects: function(){
      ...
      contact: function(args){
        if (!args) {
          ...
          pr(c('  │ ', 't-dim') + c('📸 instagram ', 't-yellow') + c(d.instagram||'https://www.instagram.com/_iamudit_02/', 't-magenta'));
          pr(c('  │', 't-dim'));
          pr(c('  └─────────────────────────────────────', 't-dim'));
          pr('');
